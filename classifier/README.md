...
The classifier implements the **classification and recommendation layer** of
Intelli3. It operates **after** the pipeline generates Multiple Intelligences
(MI) vectors and is responsible for:
...
The classifier is **fully decoupled** from ingestion, ontology instantiation,
and vector generation.

## Purpose

The Intelli3 Classifier operationalizes the final step of the Intelli3 model:

> using MI vectors to classify and recommend educational materials based on
> proximity to reference cognitive profiles.
...
Vectors follow the **fixed order**:
...
## Reference cognitive profiles

The module provides **predefined reference profiles** aligned with the research.
...
Restrict comparison to a **subset of intelligences**, simulating targeted goals.
...
```python
from classifier.loader import load_document_vectors
...
```python
from classifier.ranking import rank_documents
from classifier.profiles import PROFILES
...
## Evaluation and tests (PoC)

The module includes quantitative tests aligned with the PoC described in the
research.
...
```python
from classifier.tests import ranking_stability
